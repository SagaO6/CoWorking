...
//Importação da biblioteca responsavel pelo calendario
...
public class Reservas extends JDialog {
	...
	public Reservas() {
		...
		andarSala.setBounds(24, 254, 57, 14);
		...
		inputAndar.setBounds(95, 250, 160, 22);
		...
		ResponsavelReserva = new JLabel("Responsavel:");
		ResponsavelReserva.setBounds(392, 203, 90, 14);
		getContentPane().add(ResponsavelReserva);
		
		inputResponsavel = new JTextField();
		inputResponsavel.setBounds(476, 200, 102, 20);
		...
		inputInicioReserva = new JDateChooser(); 		  //Objeto da classe de calendário para pessoa clicar
		inputInicioReserva.setBounds(441, 254, 137, 20); //Posicionado o calendário na janela e o tamanho dele 
		getContentPane().add(inputInicioReserva);			// Adicionar o calendário na janela 

		inputFimReserva = new JDateChooser();
		inputFimReserva.setBounds(441, 300, 137, 20);
	...
	private void adicionarReserva() {
		...
		// Validação da Responsavel pela reserva
		if (inputResponsavel.getText().equals("")) {
			JOptionPane.showMessageDialog(null, "Responsavel pela reserva é obrigatório!");
			inputCategoria.requestFocus();
		}

		// Validação de início da reserva 
		/*	else if (inputResponsavel.getText().equals("")) {
			JOptionPane.showMessageDialog(null, "Código da sala obrigatório!");
			inputCod.requestFocus();
		}

		// Validação de fim da reserva 
		 ...
		 */
			...
			try {
				...
				//Formatar a datada reserva utilizando a biblioteca JCalendar para inserção CORRETA NO BANCO
